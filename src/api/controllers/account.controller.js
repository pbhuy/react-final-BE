...
module.exports = {
  ...
  activation: async (req, res, next) => {
    try {
      ...
      // decode token
      const { name, email, password, role } = jwt.verify(
        activation_token,
  ...
  refresh: async (req, res, next) => {
    try {
      ...
      jwt.verify(refresh_token, process.env.REFRESH_SECRET, (err, account) => {
        if (err) return sendErr(res, new ApiError('Please login again'));
        const access_token = accessToken({
          _id: account._id,
          role: account.role,
        });
  ...
  profile: async (req, res, next) => {
    try {
      const { id } = req.params;
      if (!id) return sendErr(res, new ApiError(400, 'Invalid account id'));
  ...
  update: async (req, res, next) => {
    try {
      ...
      // update
      const account = await Account.findByIdAndUpdate(account_id, new_account, {
        new: true,
      }).select('-password');
